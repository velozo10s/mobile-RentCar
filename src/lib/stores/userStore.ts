...
export class UserStore {
  ...
  setTokens(access: string, refresh?: string) {
    ...
    if (exp) {
      ...
      this.refreshTimer = setTimeout(async () => {
        // If no executor or no refresh token, let your 401 path handle it
        ...
        try {
          ...
          if (result?.access) {
            ...
            // backend said no -> logout; your 401 flow will also catch it
            this.logout();
          }
        } catch {
          // Swallow here; your interceptor 401 path will still work
        }
  ...
  /** Load saved auth from AsyncStorage and re-arm the proactive timer. */
